...
export class Chromagram {
  ...
  public constructor(frameSize: number, fs: number) {
    ...
    for (let i = 0; i < 12; i++) {
      this.noteFrequencies[i] = this.referenceFrequency * Math.pow(2, i / 12);
    }
      ...
      for (let octave = 1; octave <= this.numOctaves; octave++) {
        ...
        for (let harmonic = 1; harmonic <= this.numHarmonics; harmonic++) {
          let centerBin = this.round(
            (this.noteFrequencies[n] * octave * harmonic) / divisorRatio
          );
  ...
  private downSampleFrame(inputAudioFrame: number[]) {
    ...
    for (let i = 0; i < this.inputAudioFrameSize; i++) {
      filteredFrame[i] =
        inputAudioFrame[i] * b0 + x_1 * b1 + x_2 * b2 - y_1 * a1 - y_2 * a2;
      x_2 = x_1;

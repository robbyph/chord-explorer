...
const ChordRecognition = () => {
  ...
  async function chordDetection() {
    ...
    if (sourceBuffer === undefined) {
      ...
      var channelDataSilenceRemoved = await silenceRemovalAlgorithm(
        channelData
      );
  ...
  return (
    <div>
      ...
      <main className="p-6 m-2">
        <h1 className="col-span-4 text-4xl font-semibold font-HindSiliguri">
          Chord Recognition
        </h1>
        <h2 className="col-span-4 pb-4 text-2xl font-HindSiliguri">
          Powered by AI
        </h2>
        <div className="col-span-4 p-2 pl-0 m-2 mb-8 ml-0 ">
          <h2 className="col-span-4 pt-2 mb-2 text-2xl font-HindSiliguri">
            First, Upload an MP3 or WAV Audio File
          </h2>
          {submitWarning ? (
            <p className="font-bold text-red-500">
              {" "}
              Please upload an audio file before generating chord data...
            </p>
          ) : (
            ""
          )}
          ...
          {submittedIndicator ? (
            <p className="font-bold text-white">Uploaded!</p>
          ) : (
            ""
          )}
          <input
        ...
        <button
          className="col-span-4 px-2 py-2 text-xl border font-IBMPlexSans font-medium bg-white text-[#5B21B6] rounded"
          onClick={() => uploadToServer()}
        >
          Detect Chords
        </button>
        <h3 className="col-span-4 pt-2 text-xl underline font-HindSiliguri">
          Chords Detected
        </h3>
        <ul className="list-disc list-inside font-IBMPlexSans">
          {detectedChords.map((chord, i) => {
            return (
              <li key={i}>
                {chord.rootNote} {toTextQuality(chord.quality)}{" "}
                {chord.interval != 0 ? chord.interval : ""}
              </li>
            );
          })}
        </ul>
